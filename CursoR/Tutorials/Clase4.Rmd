---
title: "Clase 4:Introducción a R"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
theme: united
description: >
  Clase 4 de introducción a R.Introducción al diseño de funciones.
---
...
El ciclo for en R tiene la siguiente estructura:

![](https://www.learnbyexample.org/wp-content/uploads/r/r-for-loop-syntax.png)

### `for`loop sobre un vector

Empecemos con un ejemplo iterando sobre un vector

```{r for0,exercise=TRUE}

colors <- c("red","green","blue","yellow")

for (color in colors) {
  print(paste0("Mi color favorito es ",color))
}

```

### `for` loop sobre una lista

También podemos iterar sobre elementos de una lista:

```{r for0B, exercise=TRUE}
...
Si el `for` loop tiene solo un *statement* , podemos obviar el uso de las llaves { }:

```{r for0C, exercise=TRUE}
...
Volviendo al ejemplo de la asistencia de los equipos de fútbol, supongamos que debemos crear un reporte con el monto promedio por partido que cada equipo tuvo( con la ayuda de la función `montoPromedio` que ya calculamos) e imprimir en pantalla el resultado para cada uno.
...
## Lectura de archivos de datos en R

Hasta el momento hemos creado dataframes desde cero o hemos cargado dataframes que se encuentran en el ambiente de R.Pero que pasa si necesitamos leer datos externos a R?

### Archivos locales

Si el archivo esta ubicado localmente( es decir, dentro de la computadora donde nos encontramos trabajando), el primer paso es que debemos setear el *working directory* a la ubicación donde se encuentra dicho archivo.Para esto se utiliza la instrucción setwd("\<path_al_archivo_de_datos\>") donde \<path_al_archivo_de_datos\> es la carpeta donde se encuentra el archivo de datos, por ejemplo "/cloud/project" o "C:/Users/aacs".Notese que se debe usar "/" en lugar de "\\".primero vamos a generar el archivo desde R, guardarlo en una dirección física y luego volver a leerlo como objeto en R:

```{r local, exercise=TRUE}

#Crear dataframe 

data <- data.frame(posicion=c(1,2,3,4), 
                   nombre= c("John","Paul","Ringo","George"),
                   exitos= c(50,49,2,10)
                   )
...
#write.table(x=data,file = "data.csv",sep=",")

#leer los datos de nuevos

datosNuevos=read.table(file = "data.csv",sep=",")
...
También podemos leer archivos externos a nuestro ambiente, por ejemplo guardados en un sitio web.
...
df <- read.table(file = "https://s3.amazonaws.com/assets.datacamp.com/blog_assets/test.txt",header = FALSE)
df
...
Si un archivo de texto esta separado por "," o ";", usualmente se lee como un archivo de extensión .csv.Para esto se utiliza la función `read.csv` (aunque también funciona `read_table`):

```{r csv, exercise=TRUE}

df <- read.csv("https://s3.amazonaws.com/assets.datacamp.com/blog_assets/test.csv",header = TRUE)

df

```

#### Archivo delimitado por otro caracter
...
1.  Utilizando los datos recopilados para su grupo, guarde un dataframe en un archivo .csv y vuelva a leer el archivo guardandolo en R mediante otro nombre de objeto.Recuerde setear el working directory adecuadamente.

2.  Mediante una función, compruebe que el archivo se haya cargado con la misma estructura del objeto original.

3.  Cree una función que permita al usuario introducir dos vectores y le de la posibilidad de escoger entre restar o dividir dichos vectores.

4.  Cree una función que itere sobre el dataframe del ejercicio 1 e imprima para cada integrante del grupo el grado de ejercicio que realiza.El output de ejemplo sería\_:

-   El \<estudiante 1\> realiza \<mucho\> ejercicio.

-   El \<estudiante 2\> realiza \<poco\> ejercicio.

-   El \<estudiante n\> realiza \<algo\> ejercicio.

5.  Que función vista en clases utilizaría para que el resultado del ejercicio anterior se mejorara gramaticalmente y personalizara de acuerdo al tipo de ejercicio que realiza el estudiante?. Por ejemplo

-   El \<estudiante 1\> realiza \<mucho\> ejercicio.

-   El \<estudiante 2\> **no realiza** ejercicio.

-   El \<estudiante n\> realiza \<algo\> **de** ejercicio
